...
import { LocationSceneTypeArrayTypeCangku } from "../../Utils/constant";
export default {
  data() {
    return {
      settingCangKu: null,
  ...
  methods: {
    ...
    confirm(positionlistIndex, index, index2) {
      ...
      if (settingFlag) {
        ...
        console.log(this.settingCangKu);
        uni.$emit("updateSettingEvent", {
          settingCangKu: this.settingCangKu,
          locType: "仓库定位",
          alpha: this.settingCangKu.settings[0][0].imuAlpha,
        });
        uni.setStorageSync("settingCangKu", this.settingCangKu);
      }
    ...
    commonInit() {
      this.settingCangKu = ind.settingCangKu;
      // console.log("settingCangKu:", this.settingCangKu);
      this.refreshUI();
    },
    refreshUI() {
    ...
    refreshUIItem(positionlistIndex) {
      let settingCangKu = uni.getStorageSync(
        "settingCangKu",
        this.settingCangKu
      );
      console.log("settingCangKu:", settingCangKu);
      let settingCangKuItem =
        settingCangKu["settings"][0][positionlistIndex];
      console.log("settingCangKuItem:", settingCangKuItem);

      // 频率范围设置显示
      this.positionlist[positionlistIndex][0].positiontwo[0]["twoparameter"] =
        settingCangKuItem.frequencyStart;
      this.positionlist[positionlistIndex][0].positiontwo[1]["twoparameter"] =
        settingCangKuItem.frequencyEnd;
      this.positionlist[positionlistIndex][0].positiontwo[2]["twoparameter"] =
        settingCangKuItem.durationTime;
      this.positionlist[positionlistIndex][0].positiontwo[3]["twoparameter"] =
        settingCangKuItem.soundSpeed;

      // 基站数量设置
      this.positionlist[positionlistIndex][1].positiontwo[0]["twoparameter"] =
        settingCangKuItem.coordinate.length;

      this.positionlist[positionlistIndex][2].positiontwo = [];
      for (let i = 0; i < settingCangKuItem.coordinate.length; i++) {

        let beaconObj = {
          twoid: i,
          twoname: "基站" + (i + 1),
          twoliststare: false,
          twoparameter: settingCangKuItem.coordinate[i].join(","),
          disable: false,
        };
    ...
    fixSetting(positionlistIndex, index, index2, twoparameter) {
      ...
      // console.log(positionlistIndex, this.settingCangKu);
      // 频率范围设置显示
      if (index == 0) {
        if (index2 == 0) {
          this.settingCangKu["settings"][0][
            positionlistIndex
